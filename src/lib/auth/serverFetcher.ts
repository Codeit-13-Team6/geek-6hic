...
interface AuthSyncRequiredResponse {
  message: string;
  code: string;
  reason?: "NO_REFRESH_TOKEN" | "REFRESH_FAILED" | "TOKEN_REFRESHED";
}

interface AuthSyncRequiredError extends Error {
  response: {
    status: number;
    data: AuthSyncRequiredResponse;
  };
}
...
export const AUTH_SYNC_REQUIRED_CODE = "AUTH_SYNC_REQUIRED";
...
function createAuthSyncRequiredError(
  reason: AuthSyncRequiredResponse["reason"] = "TOKEN_REFRESHED",
): AuthSyncRequiredError {
  const error = new Error(AUTH_SYNC_REQUIRED_CODE) as AuthSyncRequiredError;
  error.response = {
    status: 401,
    data: { message: "Unauthorized", code: AUTH_SYNC_REQUIRED_CODE, reason },
  };
  return error;
}

export function isAuthSyncRequiredError(err: unknown): boolean {
  ...
  return maybeError.response?.data?.code === AUTH_SYNC_REQUIRED_CODE;
}

export function redirectToAuthSyncIfNeeded(err: unknown): void {
  if (!isAuthSyncRequiredError(err)) {
    return;
  }

  const reason = (err as {
    response?: { data?: { reason?: AuthSyncRequiredResponse["reason"] } };
  }).response?.data?.reason;

  // 게스트(리프레시 토큰 없음)는 sync를 돌릴 필요가 없습니다.
...
function collectDeferredAuthTokens(
  ...
  if (!carrier?._authSyncRequired) {
    return;
  }
  context.authSyncRequired = true;
}
...
const refreshAccessToken = async (
  ...
  if (refreshMap.has(userId)) {
    ...
    if (!forceRefresh) {
      return await refreshMap.get(userId)!;
    }
...
serverAxios.interceptors.request.use(async (config) => {
  ...
  if (!refreshToken) {
    ...
    return Promise.reject(createAuthSyncRequiredError("NO_REFRESH_TOKEN"));
  }
  ...
  if (!accessToken) {
    ...
    if (!refreshed) {
      ...
      return Promise.reject(createAuthSyncRequiredError("REFRESH_FAILED"));
    }

    accessToken = refreshed.accessToken;
    config._authSyncRequired = true;
    config._refreshedTokens = refreshed;
  }
...
serverAxios.interceptors.response.use(
  (response) => {
    if (response.config._authSyncRequired) {
      response._authSyncRequired = true;
    }
    if (response.config._refreshedTokens) {
  ...
  async (error: AxiosError) => {
    ...
    // config 없는 에러 = 직접 만든 AUTH_SYNC_REQUIRED 에러
    ...
    if (error.response?.status === 401 && !originalRequest._retry) {
      ...
      originalRequest._authSyncRequired = true;
      originalRequest._refreshedTokens = refreshed;

      const retryResponse = await serverAxios(originalRequest);
      retryResponse._authSyncRequired = true;
      retryResponse._refreshedTokens = refreshed;
...
// 서버 전용 공통 래퍼.
// authSyncMode="throw"(기본): refresh가 발생한 요청은 AUTH_SYNC_REQUIRED 에러로 승격.
// authSyncMode="response": 응답으로 넘겨 Route Handler가 코드 변환할 때 사용.
async function serverFetch<T = unknown>(
  config: AxiosRequestConfig<T>,
  options: ServerFetchOptions = {},
) {
  const { authSyncMode = "throw" } = options;

  const response = await serverAxios(config);
  if (authSyncMode === "throw" && response._authSyncRequired) {
    throw createAuthSyncRequiredError();
  }
  return response;
}

// serverAxios 는 서버단 transport
// serverFetch 는 호출 컨텍스트용 래퍼
export {
  serverAxios,
  serverFetch,
  isPublicPath,
