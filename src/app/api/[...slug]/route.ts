  ...
  try {
    ...
    if (errorResponse?.status === 401 && refreshToken) {
      try {
        ...
        const { data, status } = await axios(retryConfig);
        const response = NextResponse.json(data, { status });
        ...
        const refreshErrorResponse = getErrorResponse(refreshError);

        console.error(
          "BFF Refresh Error:",
          refreshErrorResponse?.data || getErrorMessage(refreshError),
        );

        const response = NextResponse.json(
          refreshErrorResponse?.data ?? { message: "Unauthorized" },
          { status: refreshErrorResponse?.status ?? 401 },
        );
