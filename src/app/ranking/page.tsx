...
export default async function Page() {
  const rankingPromise = getRankingBFF();
  const joinedIdsPromise = getJoinedMeetingIdsBFF().catch((error) => {
    if (isRedirectError(error)) {
      throw error;
    }

    console.error("[Ranking Joined IDs Error]:", error);
    return [];
  });

  const [rankedList, joinedIds] = await Promise.all([
    rankingPromise,
    joinedIdsPromise,
  ]);
  ...
  return (
    <div className="relative mx-auto w-full max-w-[1280px] px-6 py-10 sm:py-15 lg:py-20 2xl:px-0">
      <RankingHeroSection />
      <MyRankingSection rankedList={rankedList} joinedIds={joinedIds} />
