interface MeetingLinkSectionProps {
  link: string;
  canViewLink: boolean;
  guideText: string;
}

export function MeetingLinkSection({
  link,
  canViewLink,
  guideText,
}: MeetingLinkSectionProps) {
  return (
    <section className="w-full space-y-3 sm:space-y-4">
      <h2 className="text-[24px] font-semibold text-gray-900">모임 링크</h2>
      <div className="rounded-[20px] border border-gray-100 bg-white px-6 py-5 shadow-sm sm:rounded-[24px] sm:px-8 lg:rounded-[32px]">
        <a
          href={canViewLink ? link : undefined}
          target="_blank"
          rel="noreferrer"
          className={`block text-[16px] text-gray-700 transition ${
            canViewLink
              ? "break-all hover:text-main-green-600"
              : "pointer-events-none select-none break-all blur-[6px]"
          }`}
        >
          {link}
        </a>
        {!canViewLink ? (
          <p className="mt-2 text-sm text-gray-500 sm:text-[15px]">{guideText}</p>
        ) : null}
      </div>
    </section>
  );
}
